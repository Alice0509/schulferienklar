...
const baseHolidays = [
  ["new-year", "Neujahr", "New Year's Day", "2028-01-01"],
  ["good-friday", "Karfreitag", "Good Friday", "2028-04-14"],
  ["easter-monday", "Ostermontag", "Easter Monday", "2028-04-17"],
  ["labour-day", "Tag der Arbeit", "Labour Day", "2028-05-01"],
  ["ascension", "Christi Himmelfahrt", "Ascension Day", "2028-05-25"],
  ["pentecost-monday", "Pfingstmontag", "Whit Monday", "2028-06-05"],
  ["german-unity", "Tag der Deutschen Einheit", "German Unity Day", "2028-10-03"],
  ["christmas-day", "1. Weihnachtsfeiertag", "Christmas Day", "2028-12-25"],
  ["second-christmas-day", "2. Weihnachtsfeiertag", "Second Day of Christmas", "2028-12-26"],
];
...
function makeHoliday(code, slug, nameDe, nameEn, date, options = {}) {
  return {
    id: `${code.toLowerCase()}-public-2028-${slug}`,
...
for (const [code, name, fileStem] of states) {
  ...
  const dataset = {
    datasetName: `schulferienklar_public_holidays_${fileStem}_2028`,
    version: "0.1",
    createdAt,
    bundeslandCode: code,
    bundeslandName: name,
    ...
    sources: [
      {
        ...
        sourceUrl: "https://www.feiertage-deutschland.de/2028/",
        secondarySourceUrl: "https://www.dgb.de/service/ratgeber/feiertage/",
        trustLevel: "reference",
        lastCheckedAt: createdAt,
        notes:
          "Dates generated from nationwide German public holidays and Bundesland-specific statutory holiday mappings for 2028. Regional/local exceptions are marked separately.",
      },
    ],
    holidays,
  };

  const fileName = `${fileStem}_2028.json`;
