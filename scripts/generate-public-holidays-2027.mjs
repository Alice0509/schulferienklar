...
const baseHolidays = [
  ["new-year", "Neujahr", "New Year's Day", "2027-01-01"],
  ["good-friday", "Karfreitag", "Good Friday", "2027-03-26"],
  ["easter-monday", "Ostermontag", "Easter Monday", "2027-03-29"],
  ["labour-day", "Tag der Arbeit", "Labour Day", "2027-05-01"],
  ["ascension", "Christi Himmelfahrt", "Ascension Day", "2027-05-06"],
  ["pentecost-monday", "Pfingstmontag", "Whit Monday", "2027-05-17"],
  ["german-unity", "Tag der Deutschen Einheit", "German Unity Day", "2027-10-03"],
  ["christmas-day", "1. Weihnachtsfeiertag", "Christmas Day", "2027-12-25"],
  ["second-christmas-day", "2. Weihnachtsfeiertag", "Second Day of Christmas", "2027-12-26"],
];
...
function makeHoliday(code, slug, nameDe, nameEn, date, options = {}) {
  return {
    id: `${code.toLowerCase()}-public-2027-${slug}`,
...
for (const [code, name, fileStem] of states) {
  ...
  const dataset = {
    datasetName: `schulferienklar_public_holidays_${fileStem}_2027`,
    version: "0.1",
    createdAt,
    bundeslandCode: code,
    bundeslandName: name,
    ...
    sources: [
      {
        ...
        sourceUrl: "https://www.feiertage-deutschland.de/2027/",
        secondarySourceUrl: "https://www.schulferien.org/deutschland/feiertage/2027/",
        trustLevel: "reference",
        lastCheckedAt: createdAt,
        notes:
          "Dates generated from nationwide German public holidays and Bundesland-specific statutory holiday mappings for 2027. Regional/local exceptions are marked separately.",
      },
    ],
    holidays,
  };

  const fileName = `${fileStem}_2027.json`;
