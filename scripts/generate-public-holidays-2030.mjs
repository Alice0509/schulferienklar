...
const baseHolidays = [
  ["new-year", "Neujahr", "New Year's Day", "2030-01-01"],
  ["good-friday", "Karfreitag", "Good Friday", "2030-04-19"],
  ["easter-monday", "Ostermontag", "Easter Monday", "2030-04-22"],
  ["labour-day", "Tag der Arbeit", "Labour Day", "2030-05-01"],
  ["ascension", "Christi Himmelfahrt", "Ascension Day", "2030-05-30"],
  ["pentecost-monday", "Pfingstmontag", "Whit Monday", "2030-06-10"],
  ["german-unity", "Tag der Deutschen Einheit", "German Unity Day", "2030-10-03"],
  ["christmas-day", "1. Weihnachtsfeiertag", "Christmas Day", "2030-12-25"],
  ["second-christmas-day", "2. Weihnachtsfeiertag", "Second Day of Christmas", "2030-12-26"],
];
...
function makeHoliday(code, slug, nameDe, nameEn, date, options = {}) {
  return {
    id: `${code.toLowerCase()}-public-2030-${slug}`,
...
for (const [code, name, fileStem] of states) {
  ...
  const dataset = {
    datasetName: `schulferienklar_public_holidays_${fileStem}_2030`,
    version: "0.1",
    createdAt,
    bundeslandCode: code,
    bundeslandName: name,
    ...
    sources: [
      {
        ...
        sourceUrl: "https://www.feiertage-deutschland.de/2030/",
        secondarySourceUrl: "https://date.nager.at/PublicHoliday/Germany/2030",
        trustLevel: "reference",
        lastCheckedAt: createdAt,
        notes:
          "Dates generated from nationwide German public holidays and Bundesland-specific statutory holiday mappings for 2030. Regional/local exceptions are marked separately.",
      },
    ],
    holidays,
  };

  const fileName = `${fileStem}_2030.json`;
